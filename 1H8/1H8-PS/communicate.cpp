...
{
	if(iSvrPort > 0) m_iPort = iSvrPort;
	else m_iPort = 7000;
	if(szIPAddress != NULL) strcpy(m_szServerIP,szIPAddress);
}
...
int CNetworkServer::StartNetListenServer()
{
	...
	tv.tv_sec = SERVER_TIME_OUT; tv.tv_usec = 0;

	setsockopt(m_iServerSocketHandle, SOL_SOCKET, SO_RCVTIMEO, &tv, optlen);

	const int on=1;

	setsockopt(m_iServerSocketHandle,SOL_SOCKET,SO_REUSEADDR,&on,sizeof(on));
