 ...
 *
 *@ return "Cndev_ERROR not found!" if the corresponding message cant't be found, otherwise the corresponding message will be returned
 */
EXPORT
const char * cndevGetErrorString(cndevRet_t errorId);

#ifndef cndevCheckErrors
#define __cndevCheckErrors(err, file, line) do {                  \
cndevRet_t _err = (err);                                          \
if(CNDEV_SUCCESS != _err)                                         \
{                                                                 \
  fprintf(stderr,                                                 \
          "cndevCheckErrors(%d): %s, from file <%s>, line %i.\n", \
          _err, cndevGetErrorString(_err), basename(file), line); \
exit(1);                                                          \
}                                                                 \
} while(0)
 ...
 *
 * @ param devId the number of the card which the user selects, staring from 0
 * @ param versInfo will store a pointer to a structure which stores the information of the cars' MCU version and MLU driver version after the function ends
 *
 * @ return CNDEV_SUCCESS if success
 * @ return CNDEV_ERROR_UNINITIALIZED if the user don't call the function named cndevInit beforehand
 * @ return CNDEV_ERROR_INVALID_ARGUMENT if the parameter is NULL
 ...
 *
 * @ param devId the number of the card which the user selects, staring from 0
 * @ param powerInfo will store a pointer to a structure which stores the information of the card's power consumption after the function ends
 *
 * @ return CNDEV_SUCCESS if success
 * @ return CNDEV_ERROR_UNINITIALIZED if the user don't call the function named cndevInit beforehand
 * @ return CNDEV_ERROR_INVALID_ARGUMENT if the parameter is NULL
 ...
 *
 * @ param devId the number of the card which the user selects, staring from 0
 * @ param freqInfo will store a pointer to a structure which stores the information of the card's frequency after the function ends
 *
 * @ return CNDEV_SUCCESS if success
 * @ return CNDEV_ERROR_UNINITIALIZED if the user don't call the function named cndevInit beforehand
 * @ return CNDEV_ERROR_INVALID_ARGUMENT if the parameter is NULL
 ...
 *
 * @ param devId the number of the card which the user selects, staring from 0
 * @ param tempInfo will store a pointer to a structure which stores the information of the card's temperature after the function ends
 *
 * @ return CNDEV_SUCCESS if success
 * @ return CNDEV_ERROR_UNINITIALIZED if the user don't call the function named cndevInit beforehand
 * @ return CNDEV_ERROR_INVALID_ARGUMENT if the parameter is NULL
 ...
 *
 * @ param devId the number of the card which the user selects, staring from 0
 * @parm utilInfo will store a pointer to a structure which stores the information of the cars's utilization after the function ends
 *
 * @ return CNDEV_SUCCESS if success
 * @ return CNDEV_ERROR_UNINITIALIZED if the user don't call the function named cndevInit beforehand
 * @ return CNDEV_ERROR_INVALID_ARGUMENT if the parameter is NULL
 ...
 *
 * @ param devId the number of the card which the user selects, staring from 0
 * @ param fanInfo will store a pointer to a structure which stores the information of the cards's fan speed after the function ends
 *
 * @ return CNDEV_SUCCESS if success
 * @ return CNDEV_ERROR_UNINITIALIZED if the user don't call the function named cndevInit beforehand
 * @ return CNDEV_ERROR_INVALID_ARGUMENT if the parameter is NULL
 ...
 *
 * @ param devId the number of the card which the user selects, staring from 0
 * @ param infoCount the size of the space which the user allocates for storing the information of processes.At the same time,the parameter will store a pointer to the size of a space which is suit to store all information after the function ends
 ...
 *
 * @ param libVerInfo will store a pointer to a structure which stores the information of the cndev library version after the function ends
 *
 * @ return CNDEV_SUCCESS if success
 * @ return CNDEV_ERROR_UNINITIALIZED if the user don't call the function named cndevInit beforehand
 * @ return CNDEV_ERROR_INVALID_ARGUMENT if the parameter is NULL
 ...
 */
EXPORT
cndevRet_t cndevTopologyGetNearestDevices(int version, cndevTopologyRelationshipEnum_t rel, __uint64_t *count, __uint64_t *devIdArray, int devId);
